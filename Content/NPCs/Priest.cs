...
namespace Bismuth.Content.NPCs {
    ...
    public class Priest : ModNPC {
        ...
        public override string GetChat() {
            ...
            if (Main.dayTime && Main.LocalPlayer.GetModPlayer<Quests>().EquipmentQuest == 100 && Main.LocalPlayer.GetModPlayer<Quests>().TombstoneQuest == 0) { return Priest_1; }
            else if (!Main.dayTime && Main.LocalPlayer.GetModPlayer<Quests>().EquipmentQuest == 100 && Main.LocalPlayer.GetModPlayer<Quests>().TombstoneQuest == 0) { return Priest_2; }
            else if (!Main.dayTime && Main.LocalPlayer.GetModPlayer<Quests>().TombstoneQuest == 20 || Main.LocalPlayer.GetModPlayer<Quests>().TombstoneQuest == 30 || Main.LocalPlayer.GetModPlayer<Quests>().TombstoneQuest == 40) { return Priest_4; }
            else {
                if (!Main.dayTime) {
                    if (NPC.FindFirstNPC(NPCID.Wizard) >= 0) {
                        return WorldGen.genRand.Next(0, 2) switch {
                            0 => PriestNQ_3,
                            _ => string.Format(this.GetLocalization("Chat.PriestNQ_4").Value, Main.npc[NPC.FindFirstNPC(NPCID.Wizard)].GivenName),
                        };
                    }
                    else { return PriestNQ_3; }
                }
                else {
                    if (NPC.FindFirstNPC(NPCID.Wizard) >= 0 && WorldGen.genRand.Next(0, 3) == 0) { return string.Format(this.GetLocalization("Chat.PriestNQ_4").Value, Main.npc[NPC.FindFirstNPC(NPCID.Wizard)].GivenName); }
                    else {
                        return WorldGen.genRand.Next(0, 2) switch {
                            0 => PriestNQ_1,
                            _ => PriestNQ_2,
        ...
        public override void PostDraw(SpriteBatch spriteBatch, Vector2 screenPos, Color drawColor) {
            ...
            if (!Main.dayTime && Main.LocalPlayer.GetModPlayer<Quests>().EquipmentQuest == 100 && Main.LocalPlayer.GetModPlayer<Quests>().TombstoneQuest <= 10) { spriteBatch.Draw(available, NPC.position - Main.screenPosition + new Vector2(10, -36), Color.White); }
            if (!Main.dayTime && Main.LocalPlayer.GetModPlayer<Quests>().TombstoneQuest > 10 && Main.LocalPlayer.GetModPlayer<Quests>().TombstoneQuest < 100) { spriteBatch.Draw(active, NPC.position - Main.screenPosition + new Vector2(6, -40), Color.White); }
        }
        ...
        public override void SetChatButtons(ref string button, ref string button2) {
            ...
            if (!Main.dayTime) {
                ...
                if (Main.LocalPlayer.GetModPlayer<Quests>().EquipmentQuest == 100 && Main.LocalPlayer.GetModPlayer<Quests>().TombstoneQuest == 0) { button2 = PriestAnsv_1; }
                if (Main.LocalPlayer.GetModPlayer<Quests>().EquipmentQuest == 100 && Main.LocalPlayer.GetModPlayer<Quests>().TombstoneQuest == 5) { button2 = PriestAnsv_2; }
                if (Main.LocalPlayer.GetModPlayer<Quests>().TombstoneQuest == 30 || Main.LocalPlayer.GetModPlayer<Quests>().TombstoneQuest == 20) {
                    bool temp = false;
                    for (int num66 = 0; num66 < 58; num66++) {
                        if (Main.LocalPlayer.inventory[num66].type == ModContent.ItemType<WarriorsRemains>() && Main.player[Main.myPlayer].inventory[num66].stack > 0) {
                            temp = true;
                            button2 = PriestAnsv_3;
                        }
                    }
                    if (!temp && Main.npcChatText != Priest_5) { button2 = PriestAnsv_4; }
                }
                if (Main.LocalPlayer.GetModPlayer<Quests>().TombstoneQuest == 40) { button2 = PriestAnsv_5; }
                if (Main.LocalPlayer.GetModPlayer<Quests>().TombstoneQuest == 100) {
                    if (Main.LocalPlayer.GetModPlayer<Quests>().SoulScytheQuest == 0) {
                        bool temp = false;
                        for (int num66 = 0; num66 < 58; num66++) {
                            if (Main.LocalPlayer.inventory[num66].type == ModContent.ItemType<UnchargedSoulScythe>() && Main.LocalPlayer.inventory[num66].stack > 0) { temp = true; }
                        }
                        if (temp) { button2 = PriestAnsv_6; }
                    }
                    else if (Main.LocalPlayer.GetModPlayer<Quests>().SoulScytheQuest == 10 && Main.LocalPlayer.GetModPlayer<BismuthPlayer>().WaitSoulScythe == 1800) { button2 = PriestAnsv_7; }
                }
            }
        }
        public override void OnChatButtonClicked(bool firstButton, ref string shopName) {
            ...
            if (!Main.dayTime) {
                if (firstButton) { shopName = "PriestShop"; }
                else if (Main.LocalPlayer.GetModPlayer<Quests>().TombstoneQuest != 300) {
                    if (Main.LocalPlayer.GetModPlayer<Quests>().TombstoneQuest != 100) { quests.PriestQuests(); }
                    else { quests.SoulScytheCharging(); }
                }
            }
        }
        public override void AddShops() {
            Condition Tombstone = new(string.Format(LocUtil.Loc("Condition.CompletePriestQuests"), LocUtil.Loc("QuestsSystem.Quests.Quests.Quest8Name")), () => Main.LocalPlayer.GetModPlayer<Quests>().TombstoneQuest == 100);
        ...
        public override void AI() {
            ...
            if (QuestVariable.TombstoneStage == 200) {
                if (Main.netMode == NetmodeID.SinglePlayer) { NPC.Transform(ModContent.NPCType<PriestTeleportation>()); }
                else {
            ...
            if (Main.LocalPlayer.talkNPC != -1) {
                ...
                if (Main.npc[Main.LocalPlayer.talkNPC].type != NPC.type) {
                    if (Main.LocalPlayer.GetModPlayer<Quests>().TombstoneQuest < 10) { Main.LocalPlayer.GetModPlayer<Quests>().TombstoneQuest = 0; }
                }
            }
            else {
                if (Main.LocalPlayer.GetModPlayer<Quests>().TombstoneQuest < 10) { Main.LocalPlayer.GetModPlayer<Quests>().TombstoneQuest = 0; }
