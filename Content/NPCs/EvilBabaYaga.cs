...
namespace Bismuth.Content.NPCs {
    ...
    public class EvilBabaYaga : ModNPC {
        ...
        public override void SetStaticDefaults() => Main.npcFrameCount[NPC.type] = 31;
        ...
        public override void AI() {
            ...
            if (currentphase == 1) {
                ...
                if (NPC.ai[0] % 30 + Main.rand.Next(-15, 10) == 0) {
                    ...
                    while (!WorldGen.SolidTile(Blastpos.ToTileCoordinates().X, Blastpos.ToTileCoordinates().Y)) { Blastpos.Y++; }
                    Blastpos.Y -= 25;
                    Projectile.NewProjectile(NPC.GetSource_FromThis(), Blastpos, new Vector2(0, 0), ModContent.ProjectileType<BabaYagaPreBlast>(), 0, 0f);

                }
                if (NPC.ai[0] % 70 + Main.rand.Next(-10, 10) == 0) { NPC.NewNPC(NPC.GetSource_FromThis(), (int)NPC.Center.X, (int)NPC.Center.Y, ModContent.NPCType<SwampWraith>()); }
                if (NPC.ai[0] % 16 == 0) {
                    Player player = Main.LocalPlayer;
        ...
        public void ChoosePhase() {
            if (dead) {
                ...
                CameraSystem.FocusOn(ModContent.NPCType<SwampWitchDeath>());
            ...
            if (currentphase == 1 && NPC.life > (NPC.lifeMax / 2)) { currentphase = 2; }
            else if (currentphase == 1 && NPC.life <= (NPC.lifeMax / 2)) { currentphase = 3; }
            else if (currentphase == 2 || currentphase == 3 || currentphase == 0) { currentphase = 1; }
            UpdateDirection();
        }
        public override void FindFrame(int frameHeight) {
            ...
            if (currentphase == 1) {
                ...
                if (tick == 5) {
                    tick = 0;
                    if (currentframe == 0) { currentframe = 21; }
            ...
            if (currentphase == 2) {
                ...
                if (tick == 5) {
                    tick = 0;
                    if (currentframe < 20) { currentframe++; }
                    if (currentframe == 20) {
                        currentframe = 0;
                        ChoosePhase();
                    }
