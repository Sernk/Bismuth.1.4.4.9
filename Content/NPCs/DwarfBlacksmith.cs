...
namespace Bismuth.Content.NPCs {
    ...
    public class DwarfBlacksmith : ModNPC {
        ...
        public override string GetChat() {
            ...
            IQuest quest = QuestRegistry.GetAvailableQuests(player, "DwarfBlacksmith").FirstOrDefault();

            if (NPC.AnyNPCs(NPCID.GoblinTinkerer) && Main.LocalPlayer.GetModPlayer<Quests>().GlamdringQuest == 0 && Main.LocalPlayer.GetModPlayer<Quests>().EquipmentQuest == 100) { return string.Format(this.GetLocalization("Chat.Blacksmith_1").Value, Main.LocalPlayer.name); }
            if (TempNPCs.DwarfBlacksmithNewQuest && quest != null) { return quest.GetChat(NPC, player); }
            else if (Main.LocalPlayer.GetModPlayer<Quests>().GlamdringQuest == 20) { return Blacksmith_3; }
            else if (Main.LocalPlayer.GetModPlayer<Quests>().MinotaurHornQuest == 0 && Main.LocalPlayer.GetModPlayer<Quests>().GlamdringQuest == 100 && Main.LocalPlayer.GetModPlayer<BismuthPlayer>().KilledEoC) { return Blacksmith_7; }
            else if (Main.LocalPlayer.GetModPlayer<Quests>().MinotaurHornQuest == 20) { return Blacksmith_9; }
            else if (Main.LocalPlayer.GetModPlayer<Quests>().ArmorPlateQuest == 20) { return Blacksmith_14; }
            else if (Main.LocalPlayer.GetModPlayer<Quests>().ArmorPlateQuest == 0 && Main.LocalPlayer.GetModPlayer<Quests>().GlamdringQuest == 100 && Main.LocalPlayer.GetModPlayer<BismuthPlayer>().KilledWoF) { return Blacksmith_12; }
            else {
                if (NPC.FindFirstNPC(NPCID.Demolitionist) >= 0 && WorldGen.genRand.Next(0, 4) == 0) { return string.Format(this.GetLocalization("Chat.BlacksmithNQ_2").Value, Main.npc[NPC.FindFirstNPC(NPCID.Demolitionist)].GivenName); }
        ...
        public override void AddShops() {
            Condition KilledEoCS = Condition.DownedEyeOfCthulhu;
            Condition KilledSkeletron = Condition.DownedSkeletron;
        ...
        public override void SetChatButtons(ref string button, ref string button2) {
            ...
            if (NPC.AnyNPCs(NPCID.GoblinTinkerer) && Main.LocalPlayer.GetModPlayer<Quests>().GlamdringQuest == 0 && Main.LocalPlayer.GetModPlayer<Quests>().EquipmentQuest == 100) { button2 = BlacksmithAnsv_1; }
            if (Main.LocalPlayer.GetModPlayer<Quests>().GlamdringQuest == 5) { button2 = BlacksmithAnsv_2; }
            if (Main.LocalPlayer.GetModPlayer<Quests>().GlamdringQuest == 20) {
                bool temp = false;
                for (int num66 = 0; num66 < 58; num66++) {
                    if (Main.LocalPlayer.inventory[num66].type == ModContent.ItemType<GlamdringBlueprint>() && Main.LocalPlayer.inventory[num66].stack > 0) { temp = true; }
                }
                if (temp) { button2 = BlacksmithAnsv_3; }
            }
            if (Main.LocalPlayer.GetModPlayer<Quests>().GlamdringQuest == 30 && Main.npcChatText != Blacksmith_5) { button2 = BlacksmithAnsv_5; }
            if (Main.LocalPlayer.GetModPlayer<Quests>().MinotaurHornQuest == 0 && Main.LocalPlayer.GetModPlayer<Quests>().GlamdringQuest == 100 && Main.LocalPlayer.GetModPlayer<BismuthPlayer>().KilledEoC) { button2 = BlacksmithAnsv_6; }
            if (Main.LocalPlayer.GetModPlayer<Quests>().MinotaurHornQuest == 5) { button2 = BlacksmithAnsv_7; }
            if (Main.LocalPlayer.GetModPlayer<Quests>().MinotaurHornQuest == 20) {
                bool temp = false;
                for (int num66 = 0; num66 < 58; num66++) {
                    if (Main.LocalPlayer.inventory[num66].type == ModContent.ItemType<MinotaurHorn>() && Main.LocalPlayer.inventory[num66].stack > 0) { temp = true; }
                }
                if (temp) { button2 = BlacksmithAnsv_8; }
                else if (!temp && Main.npcChatText != Blacksmith_10) { button2 = BlacksmithAnsv_9; }
            }
            if (Main.LocalPlayer.GetModPlayer<Quests>().ArmorPlateQuest == 0 && Main.LocalPlayer.GetModPlayer<Quests>().MinotaurHornQuest == 100 && Main.LocalPlayer.GetModPlayer<BismuthPlayer>().KilledWoF) { button2 = BlacksmithAnsv_10; }
            if (Main.LocalPlayer.GetModPlayer<Quests>().ArmorPlateQuest == 5) { button2 = BlacksmithAnsv_11; }
            if (Main.LocalPlayer.GetModPlayer<Quests>().ArmorPlateQuest == 20) {
                bool temp = false;
                for (int num66 = 0; num66 < 58; num66++) {
                    if (Main.LocalPlayer.inventory[num66].type == ModContent.ItemType<DwarvenBrokenArmor>() && Main.LocalPlayer.inventory[num66].stack >= 5) { temp = true; }
                }
                if (temp) { button2 = BlacksmithAnsv_13; }
                else if (!temp && Main.npcChatText != Blacksmith_15) { button2 = BlacksmithAnsv_12; }
            }
            if (Main.LocalPlayer.GetModPlayer<Quests>().ArmorPlateQuest <= 80) { TempNPCs.DwarfBlacksmithNewQuest = true; }
            if (TempNPCs.DwarfBlacksmithNewQuest) {
                Player player = Main.LocalPlayer;
                IQuest quest = QuestRegistry.GetAvailableQuests(player, "DwarfBlacksmith").FirstOrDefault();
                if (quest == null) {
                    for (int num66 = 0; num66 < 58; num66++) {
                        if (Main.LocalPlayer.inventory[num66].type == ModContent.ItemType<DwarvenBrokenArmor>() && Main.LocalPlayer.inventory[num66].stack > 0) { button2 = BlacksmithAnsv_14; }
                    }
                }
                if (TempNPCs.DwarfBlacksmithNewQuest && quest != null) { button2 = quest.GetButtonText(player); }
            }
        }
        ...
        public override void AI() {
            if (Main.netMode != NetmodeID.Server) {
                if ((currentframe == 0 && tick == 0) || (currentframe == 10 && tick == 0)) {
                    SoundStyle anvilStrike = new("Bismuth/Sounds/Custom/AnvilStrike") {
            ...
            if ((currentframe >= 0 && currentframe <= 1) || (currentframe >= 10 && currentframe <= 11)) { Lighting.AddLight(NPC.position, new Vector3(242, 138, 48) * 0.0055f); }
            else { Lighting.AddLight(NPC.position, new Vector3(242, 138, 48) * 0.0045f); }
            if (Main.LocalPlayer.talkNPC != -1) {
                if (Main.npc[Main.LocalPlayer.talkNPC].type != NPC.type) {
                    if (Main.LocalPlayer.GetModPlayer<Quests>().GlamdringQuest < 10) { Main.LocalPlayer.GetModPlayer<Quests>().GlamdringQuest = 0; }
                    if (Main.LocalPlayer.GetModPlayer<Quests>().MinotaurHornQuest < 10) { Main.LocalPlayer.GetModPlayer<Quests>().MinotaurHornQuest = 0; }
                    if (Main.LocalPlayer.GetModPlayer<Quests>().ArmorPlateQuest < 10) { Main.LocalPlayer.GetModPlayer<Quests>().ArmorPlateQuest = 0; }
                }
            }
            else {
                if (Main.LocalPlayer.GetModPlayer<Quests>().GlamdringQuest < 10) { Main.LocalPlayer.GetModPlayer<Quests>().GlamdringQuest = 0; }
        ...
        public override void FindFrame(int frameHeight) {
            ...
            if (Main.LocalPlayer.talkNPC != -1) {
                if (Main.npc[Main.LocalPlayer.talkNPC].whoAmI == NPC.whoAmI) {
                    ...
                    if (currentframe > 19) { currentframe = 10; }
                }
            }
            else {
                if (currentframe < 10) { currentframe = 10; }
