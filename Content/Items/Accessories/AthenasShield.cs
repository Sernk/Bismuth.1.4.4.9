...
namespace Bismuth.Content.Items.Accessories
{
    ...
    public class AthenasShield : ModItem
    {
        ...
        public override void UpdateAccessory(Player player, bool hideVisual)
        {
            ...
            if (progress == 2)
            {
                ...
                AthenasShieldPlayer.Heal = 0.25f;
            }
            if (progress == 3)
            {
                Item.defense = 5;
                ...
                AthenasShieldPlayer.Heal = 0.35f;
            }
            if (progress == 4)
            {
                Item.defense = 7;
    ...
    public class AthenasShieldPlayer : ModPlayer
    {
        public static float Heal = 0.35f;

        int progress = LocalizationSystem.GetProgress();

        public override void UpdateLifeRegen()
        {
            if (Player.GetModPlayer<BismuthPlayer>().IsEquippedAthenasShield && Player.statLifeMax2 <= 200 && progress == 5)
            {
                Player.lifeRegen += 30;
                Player.lifeRegenTime = Math.Max(Player.lifeRegenTime, 2f);
            }
            if (Player.GetModPlayer<BismuthPlayer>().IsEquippedAthenasShield && Player.statLifeMax2 <= 150 && progress >= 6)
            {
                Player.lifeRegen += 30;
                Player.lifeRegenTime = Math.Max(Player.lifeRegenTime, 2f);
            }
        ...
        {
            if (Player.GetModPlayer<BismuthPlayer>().IsEquippedAthenasShield && info.Damage > 0)
            {
                int heal = (int)(info.Damage * Heal);

                heal = Math.Min(heal, 30);
