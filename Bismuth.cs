...
namespace Bismuth {
    public class Bismuth : Mod {
        ...
        public static Vector2 screenHalf => new Vector2((float)Main.screenWidth, (float)Main.screenHeight) * 0.5f;
        public static Vector2 Random2(float max) => Random2(-max, max, -max, max);
        public static Vector2 Random2(float minX, float maxX, float minY, float maxY) => new Vector2(Main.rand.NextFloat(minX, maxX), Main.rand.NextFloat(minY, maxY));
        public static void CameraUpdate(bool setValues = true) {
            if (screenShakes.Count > 0) {
                for (int index = 0; index < screenShakes.Count; ++index) {
                    ScreenShake screenShake = screenShakes[index];
                    float f = screenShake.strength;
                    Vector2 vector2_3 = Random2(f * 0.5f) * ((float)screenShake.time / (float)screenShake.timeMax) * 1;
                    cameraShakeOffset = new Vector2((float)(int)((double)vector2_3.X / (double)Main.GameZoomTarget), (float)(int)((double)vector2_3.Y / (double)Main.GameZoomTarget));
                    ...
                    if (setValues) {
                        ...
                        if ((double)screenShake.time <= 0.0) {
                            screenShakes.RemoveAt(index);
                            --index;
                        }
                        else { screenShakes[index] = screenShake; }
                    }
                }
            }
        }
        public override void HandlePacket(BinaryReader reader, int whoAmI) {
            ...
            switch (msgType) {
                ...
                case 15: HandlePacket(reader, ref BismuthWorld.FirstTotemDeactivation); break;
                case 16: HandlePacket(reader, ref BismuthWorld.TotemCooldown); break;
            }
        ...
        public override void Load() {
            ...
            if (!Main.dedServ) {
                ...
                TextureAssets.Projectile[ProjectileID.Hamdrax] = ModContent.Request<Texture2D>("Bismuth/Resprites/Drax_proj");
            }
        ...
        public override void Unload() {
            ...
            string TerrariaPatchI(int item) => $"Terraria/Images/Item_{item}";
            string TerrariaPatchP(int proj) => $"Terraria/Images/Projectile_{proj}";
            TextureAssets.Item[ItemID.Excalibur] = ModContent.Request<Texture2D>(TerrariaPatchI(368));
            TextureAssets.Item[ItemID.Drax] = ModContent.Request<Texture2D>(TerrariaPatchI(579));
            TextureAssets.Item[ItemID.HallowedRepeater] = ModContent.Request<Texture2D>(TerrariaPatchI(578));
            TextureAssets.Item[ItemID.PickaxeAxe] = ModContent.Request<Texture2D>(TerrariaPatchI(990));
            TextureAssets.Item[ItemID.LightDisc] = ModContent.Request<Texture2D>(TerrariaPatchI(561));
            TextureAssets.Projectile[ProjectileID.LightDisc] = ModContent.Request<Texture2D>(TerrariaPatchP(106));
            TextureAssets.Projectile[ProjectileID.Hamdrax] = ModContent.Request<Texture2D>(TerrariaPatchI(107));
        }
